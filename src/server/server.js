...
//Jest does not cleanly exit, because it listens to the same port as the server. 
//We need to split the server side code into 2 files: one that sets up the endpoints and another one that starts the server. Then test only the file that sets the endpoints.